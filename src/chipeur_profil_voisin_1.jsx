    ...
    <div style={{ background: C.card, flexShrink: 0, borderBottom: `1px solid ${C.border}` }}>
      <div style={{ padding: "12px 16px 8px", display: "flex", alignItems: "flex-start", gap: 12 }}>
        <div style={{ position: "relative", flexShrink: 0 }}>
          ...
          <div style={{ position: "absolute", bottom: -6, left: "50%", transform: "translateX(-50%)", background: "linear-gradient(135deg,#FF5733,#F7A72D)", color: "#fff", fontSize: 8, fontWeight: 700, padding: "2px 7px", borderRadius: 8, whiteSpace: "nowrap", border: `2px solid ${C.card}` }}>✦ Pépite du Quartier</div>
        </div>
        <div style={{ flex: 1, paddingTop: 2 }}>
          <div style={{ fontFamily: syne, fontWeight: 700, fontSize: 16, color: C.ink }}>Lucas M.</div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 6 }}>
            ...
            <div style={{ flex: 1, height: 6, background: C.pill, borderRadius: 3, overflow: "hidden" }}><div style={{ height: "100%", width: "72%", borderRadius: 3, background: "linear-gradient(90deg,#FF5733,#F7A72D)" }} /></div>
      ...
      <div style={{ display: "flex", padding: "6px 16px 8px" }}>
        {[{ n: "24", l: "posts" }, { n: "138", l: "abonnés" }, { n: "#3", l: "classement", color: C.gold }, { n: "720", l: "XP ce mois", color: C.accent }].map((s, i) => <div key={i} style={{ display: "contents" }}>{i > 0 && <div style={{ width: 1, background: C.border, margin: "3px 0" }} />}<div style={{ flex: 1, textAlign: "center" }}><div style={{ fontFamily: syne, fontWeight: 700, fontSize: 17, color: s.color || C.ink }}>{s.n}</div><div style={{ fontSize: 9, color: C.ink2, marginTop: 1 }}>{s.l}</div></div></div>)}
      </div>
...
function TabDefis() {
  const defis = [{ icon: "👟", title: "Défi Sneakers", sub: "Post publié il y a 12 min", stat: "🔥 18 · 👁 84 vues", xp: "+15 XP", grad: "linear-gradient(135deg,#FF5733,#F7A72D)" }, { icon: "🧥", title: "Vintage Revival", sub: "Post publié hier", stat: "💛 9 · 👁 41 vues", xp: "+15 XP", grad: "linear-gradient(135deg,#5B2D8E,#9B59B6)" }, { icon: "🌿", title: "Look Éco-Responsable", sub: "Terminé · Non participé", stat: "—", xp: "0 XP", ended: true }];
...
function TabRewards() {
  ...
  return <>
    <div style={{ background: "linear-gradient(135deg,#1A1714,#3D3530)", borderRadius: 18, padding: 16, marginBottom: 14, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", right: -8, top: -8, fontSize: 64, opacity: 0.12 }}>🏆</div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}><span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }}>Classement · Avril 2026</span><span style={{ fontSize: 10, background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.7)", padding: "2px 8px", borderRadius: 8 }}>28 voisins actifs</span></div><div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={{ fontFamily: syne, fontWeight: 700, fontSize: 44, color: C.gold, lineHeight: 1 }}>#3</div><div><div style={{ fontFamily: syne, fontWeight: 700, fontSize: 18, color: "#fff" }}>720 XP ce mois</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", marginTop: 2 }}>Encore 80 XP pour passer #2</div></div></div><div style={{ height: 6, background: "rgba(255,255,255,0.15)", borderRadius: 3, overflow: "hidden", marginTop: 10 }}><div style={{ height: "100%", width: "72%", borderRadius: 3, background: "linear-gradient(90deg,#FF5733,#F7A72D)" }} /></div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "rgba(255,255,255,0.5)", marginTop: 4 }}><span>0</span><span>Top 3 → bon d'achat</span><span>1000 XP</span></div></div>
    ...
    {lb.map((l, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, background: l.me ? "#FFF8F6" : C.card, borderRadius: 14, padding: "10px 12px", marginBottom: 6, border: `1px solid ${l.me ? "rgba(232,73,10,0.3)" : C.border}` }}><div style={{ fontFamily: syne, fontWeight: 700, fontSize: 15, color: l.rc || C.ink2, width: 22, textAlign: "center" }}>{l.r}</div><div style={{ width: 34, height: 34, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, background: l.bg }}>{l.av}</div><div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, color: C.ink }}>{l.n}</div><div style={{ fontSize: 10, color: C.ink2 }}>{l.lv}</div></div><div style={{ fontFamily: syne, fontWeight: 700, fontSize: 13, color: C.accent }}>{l.xp}</div></div>)}
    ...
    <div style={{ background: "linear-gradient(135deg,#0A3D2E,#1D9E75)", borderRadius: 16, padding: 14, marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}><div style={{ fontFamily: syne, fontWeight: 700, fontSize: 28, color: "#fff" }}>10€</div><div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, color: "#fff" }}>Secondhand Co.</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.7)", marginTop: 2 }}>Récompense Top 3 · Mars 2026</div><div style={{ fontSize: 9, color: "rgba(255,255,255,0.6)", marginTop: 4 }}>Expire le 31 mai 2026</div></div><button style={{ background: "rgba(255,255,255,0.2)", color: "#fff", border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: dm }}>CODE</button></div>
