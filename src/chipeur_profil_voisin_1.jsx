...
function StatusBar() {
  return (
    <div style={{
      ...
      flexShrink: 0, position: "absolute", top: 0, left: 0, right: 0, zIndex: 10,
    }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
      <div style={{
        ...
        <button onClick={onBack} style={{
          width: 32, height: 32, background: C.pill, borderRadius: "50%",
        ...
        <div style={{
          ...
          background: "#FFF0EB", padding: "3px 8px", borderRadius: 8,
        }}>+5 XP</div>
      </div>
      <div style={{
        ...
        <div style={{ fontFamily: syne, fontWeight: 700, fontSize: 17, color: C.ink, textAlign: "center", lineHeight: 1.3 }}>
          {defi.q}
        </div>
        ...
        <div style={{ display: "flex", gap: 8, width: "100%" }}>
          {["text", "photo"].map(m => (
            <button key={m} onClick={() => setMode(m)} style={{
              flex: 1, padding: 9, borderRadius: 12, fontSize: 12, fontWeight: 600,
              border: `1.5px solid ${mode === m ? C.accent : C.border}`,
              background: mode === m ? C.accent : C.card,
              color: mode === m ? "#fff" : C.ink2, cursor: "pointer", fontFamily: dm,
            }}>{m === "text" ? "✏️ Texte" : "📷 Photo"}</button>
          ))}
        </div>
        {mode === "text" && (
          <>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center", width: "100%" }}>
              {defi.sugs.map(s => (
                <button key={s} onClick={() => setValue(s)} style={{
                  ...
                  background: value === s ? C.accent : C.pill,
                  color: value === s ? "#fff" : C.ink2,
        ...
        {mode === "photo" && (
          <div style={{
            width: "100%", aspectRatio: "4/3", background: C.pill, borderRadius: 16,
            border: "2px dashed rgba(255,87,51,0.3)",
            display: "flex", flexDirection: "column",
            alignItems: "center", justifyContent: "center",
            gap: 6, cursor: "pointer", fontSize: 12, color: C.ink2,
          }}>
      ...
      <button
        onClick={() => { setSaved(true); setTimeout(() => onSave(value), 600); }}
        style={{
          position: "absolute", bottom: 90, left: 20, right: 20, zIndex: 10,
          background: saved ? C.pro : C.accent,
          color: "#fff", border: "none", borderRadius: 16, padding: 14,
          fontSize: 14, fontWeight: 700, fontFamily: dm, cursor: "pointer",
          textAlign: "center", transition: "background 0.3s",
        }}
      >{saved ? "✓ Enregistré !" : "Enregistrer · +5 XP ✓"}</button>
    </div>
  );
}
...
function EditProfileScreen({ onBack }) {
  ...
  const inp = {
    ...
    border: `1.5px solid ${C.border}`, fontFamily: dm,
    fontSize: 13, color: C.ink, background: C.card, outline: "none", boxSizing: "border-box",
  };
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
      <div style={{
        ...
        <button
          onClick={() => { setSaved(true); setTimeout(onBack, 800); }}
          style={{
            background: saved ? C.pro : C.accent, color: "#fff", border: "none",
            borderRadius: 20, padding: "6px 16px", fontSize: 12, fontWeight: 700,
            fontFamily: dm, cursor: "pointer", transition: "background 0.3s",
          }}
        >{saved ? "✓ Sauvé" : "Sauver"}</button>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 18px" }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 24 }}>
          <div style={{
            width: 80, height: 80, borderRadius: "50%", background: "#E8F4FD",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 36, border: `3px solid ${C.gold}`, position: "relative",
          }}>
            🧑‍🦱
            <div style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 13, color: "#fff", border: `2px solid ${C.card}`, cursor: "pointer",
            }}>📷</div>
          </div>
          <div style={{ fontSize: 11, color: C.accent, fontWeight: 600, marginTop: 8, cursor: "pointer" }}>Changer la photo</div>
        </div>
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11, fontWeight: 600, color: C.ink2, marginBottom: 5, display: "block" }}>Prénom</label>
          <input value={name} onChange={e => setName(e.target.value)} style={inp} />
        </div>
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11, fontWeight: 600, color: C.ink2, marginBottom: 5, display: "block" }}>Ville</label>
          <input value={loc} onChange={e => setLoc(e.target.value)} style={inp} />
        </div>
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11, fontWeight: 600, color: C.ink2, marginBottom: 5, display: "block" }}>Bio</label>
          <textarea value={bio} onChange={e => setBio(e.target.value)} rows={3} style={{ ...inp, resize: "none", lineHeight: 1.5 }} />
        </div>
        <div style={{
          fontSize: 10, color: C.ink2, lineHeight: 1.5,
...
function ProfileHeader({ activeTab, onTabChange, onEditProfile, setPage }) {
  ...
  return (
    <div style={{ background: C.card, flexShrink: 0, borderBottom: `1px solid ${C.border}` }}>
      ...
      <div style={{
        height: 100,
        background: "linear-gradient(135deg, #1A1A2E 0%, #FF5733 60%, #F7A72D 100%)",
      ...
      <div style={{ padding: "0 16px 0", position: "relative" }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: "#E8F4FD",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 30, border: `3px solid ${C.card}`,
            boxShadow: "0 2px 12px rgba(26,23,20,0.15)",
          }}>🧑‍🦱</div>
          {/* Anneau XP coloré */}
          <div style={{
            ...
            background: `linear-gradient(${C.card}, ${C.card}) padding-box, linear-gradient(135deg,#FF5733,#F7A72D) border-box`,
          }} />
        </div>

        {/* Boutons action — à droite de l'avatar */}
        <div style={{
          ...
          <button onClick={onEditProfile} style={{
            ...
            fontFamily: dm, cursor: "pointer", boxShadow: "0 1px 4px rgba(0,0,0,0.08)",
          }}>✏️ Modifier</button>
          <button style={{
        ...
        <div style={{ marginTop: 8 }}>
          <div style={{ fontFamily: syne, fontWeight: 700, fontSize: 18, color: C.ink, lineHeight: 1 }}>
            Lucas M.
          </div>
        ...
        <div style={{ marginTop: 10, marginBottom: 12 }}>
          ...
          <div style={{ height: 5, background: C.pill, borderRadius: 3, overflow: "hidden" }}>
            <div style={{
              height: "100%", width: "72%", borderRadius: 3,
              background: "linear-gradient(90deg,#FF5733,#F7A72D)",
            }} />
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        {[
          ...
          <div key={i} style={{ flex: 1, textAlign: "center", borderRight: i < 3 ? `1px solid ${C.border}` : "none" }}>
            <div style={{ fontFamily: syne, fontWeight: 700, fontSize: 16, color: s.color || C.ink }}>
              {s.n}
            </div>
      ...
      {/* Bandeau réductions */}
      <div onClick={() => setPage("reductions")} style={{ padding: "10px 16px", cursor: "pointer" }}>
        <div style={{
          display: "flex", alignItems: "center", gap: 10,
        ...
        <div key={p.id} style={{
          ...
          <button
            ...
            style={{
              ...
              width: 22, height: 22, background: "rgba(26,23,20,0.55)",
          ...
          <div style={{
            ...
            {p.pepite && (
              <span style={{
                ...
                borderRadius: 6, padding: "2px 6px", fontSize: 9, color: "#fff", fontWeight: 700,
              }}>✨ Pépite</span>
            )}
          </div>
        </div>
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <div style={{
          ...
          background: "#FFF0EB", padding: "3px 10px", borderRadius: 8,
        }}>+5 XP par réponse</div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {items.map((it, i) => (
          <div key={i} onClick={() => onOpen(i)} style={{
            background: it.done ? C.card : "transparent",
            borderRadius: 16, overflow: "hidden", cursor: "pointer",
            border: it.done ? `1px solid ${C.border}` : "1px dashed rgba(255,87,51,0.25)",
            padding: it.hasPhoto ? 0 : 12,
          }}>
            {it.hasPhoto ? (
              <>
                <div style={{
                  ...
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 36, background: it.photoBg,
                }}>{it.photoEmoji}</div>
...
function TabDefis() {
  ...
  return (
    <>
      {defis.map((d, i) => (
        <div key={i} style={{
          background: C.card, borderRadius: 16, marginBottom: 8,
          overflow: "hidden", border: `1px solid ${C.border}`,
          opacity: d.ended ? 0.5 : 1,
        }}>
          <div style={{
            padding: "12px 14px", display: "flex", alignItems: "center", gap: 12,
            background: d.ended ? C.pill : d.grad,
          }}>
            <div style={{ fontSize: 22 }}>{d.icon}</div>
            <div>
              <div style={{ fontFamily: syne, fontWeight: 700, fontSize: 13, color: d.ended ? C.ink : "#fff" }}>
                {d.title}
              </div>
              <div style={{ fontSize: 10, color: d.ended ? C.ink2 : "rgba(255,255,255,0.8)", marginTop: 1 }}>
                {d.sub}
              </div>
            </div>
          </div>
          <div style={{ padding: "8px 14px 10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <span style={{
              ...
              color: d.ended ? C.ink2 : C.accent,
              background: d.ended ? C.pill : "#FFF0EB",
            }}>{d.xp}</span>
          </div>
        </div>
      ))}
...
function TabRewards() {
  ...
  return (
    <>
      <div style={{
        background: "linear-gradient(135deg,#1A1714,#3D3530)",
      ...
      {lb.map((l, i) => (
        <div key={i} style={{
          display: "flex", alignItems: "center", gap: 10,
          background: l.me ? "#FFF8F6" : C.card,
          borderRadius: 14, padding: "10px 12px", marginBottom: 6,
          border: `1px solid ${l.me ? "rgba(255,87,51,0.3)" : C.border}`,
        }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {tr.map((t, i) => (
          <div key={i} style={{
            background: t.w ? "#FFFBF0" : C.card, borderRadius: 14, padding: 12,
            border: `1px solid ${t.w ? "rgba(247,167,45,0.4)" : C.border}`,
            textAlign: "center", opacity: t.w ? 1 : 0.5,
          }}>
      ...
      {screen === "profil" && (
        <>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px 12px" }}>
            {activeTab === "Posts" && (
              <TabPosts posts={posts} onDelete={id => setDeleteTarget(id)} />
            )}
            {activeTab === "Mon univers" && (
              <TabUnivers items={univers} onOpen={i => { setMiniDefiIdx(i); setScreen("minidefi"); }} />
            )}
